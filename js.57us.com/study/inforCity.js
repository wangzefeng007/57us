function comSelect(){
	$(document).on("click",".gf-select > span",function(){
		$(this).closest(".gf-select").css("z-index",100).addClass("on");
		$(".gf-select ul").hide();
	...
	$(document).on("click",".gf-select ul li",function(){
		...
		select.css("z-index",1).removeClass("on");
	...
	$(document).on("click",function(e){
		if($(e.target).closest(".gf-select").length == 0){
			$(".gf-select").css("z-index",1).removeClass("on");
			$(".gf-select ul").hide();
		}
	});
}
function selectCity(options){
	...
	if(suffix == 'cn') {
		var jsallcity = "http://js.57us.cn/study/allcity.js";
	} else if(suffix == 'com') {
		var jsallcity = "http://js.57us.com/study/allcity.js";
	}else if(suffix == 'net'){
		var jsallcity = "http://js.57us.net/study/allcity.js";
	}
	var ajaxConfig = {
		url : jsallcity,
		dataType : "jsonp",
