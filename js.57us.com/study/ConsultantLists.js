...
$(function () {
    ...
    //服务项目
    $('#ServiceProject').empty();
    $.get('/consultant/getproject', function(data) {
        var item;
        $.each(data, function(i, list) {
            item = '<li>' +
                '<label name="cbt3" type="checkbox" val="' + list.id + '" class="cbt"><i></i>' + list.name + '</label>' +
                '</li>';
            $('#ServiceProject').append(item);
        });

        var Filter = {
            'idname': 'ServiceProject',
            'nini': 'ServiceProjectAll',
            'dataidname': 'ServiceProjectName',
            'ajaxname': 'AjaxServiceProject',
        }
        TourCheckbox(Filter);
    },'json');
    ...
    $.get('/consultant/getcity', function(data) {
        ...
        $.each(data, function(i, list) {
            item = '<li>' +
                '<label name="cbt2" type="checkbox" val="' + list.AeraID + '" class="cbt"><i></i>' + list.name + '</label>' +
                '</li>';
            $('#Region').append(item);
        });
        var Filter = {
...
function Ajax(Page) {
    ...
    //服务项目
    var ServiceProject = [];
    $('#AjaxServiceProject span').each(function () {
        ServiceProject.push($(this).attr("data-id"));
    })
    //选择区域
    var Region = [];
    ...
    $.ajax({
        ...
        success: function(data) {	//函数回调
            if(data.ResultCode == "200"){
                ...
                $("#Position").append(' > 搜索“'+Keyword+'”结果');
                $("#Nosearch").empty();
                $("#Nosearch").append('sorry，没有找到“ <span>'+Keyword+'</span>”相关的顾问！');
                // $("#Filter").hide();
                $(".Sequence").hide();
                DataFailure(data);
            }
...
function DataSuccess(data) {
    ...
    $.each(data.Data, function(i, list) {
        item =  '<li data-id="'+list.StudyID+'">' +
            '<a href="'+list.StudyUrl+'" target="_blank" title="'+list.Study_name+'">' +
            '<p class="img">' +
            ...
            '<p class="counIns mt20"><span class="green">顾问简介：</span>'+list.StudyDepict+'</p>' +
            '</a>' +
            '</li>';
