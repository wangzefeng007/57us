/**
 * Created by Foliage on 2016/11/28.
 */
/**
...
$(function () {
    ...
    $(document).on('click','.Selection',function () {
        ...
        if(Times > 0){
            window.location = m_url+'/study/marryconsultantthree/?MarryID='+MarryID+'&ConsultantID='+ConsultantID;
        }else {
            $.toast('您选择的顾问已经超过五个');
        }
    })
...
//ajax提交参数
var ajaxData = {
    'Intention':'ConsultantMatchingDetail', //方法
                ...
                if(data.ResultCode == '200' || data.ResultCode == '102') {
                    if(data.Data.length) {
                        ...
                        $.each(data.Data, function(i, list) {
                            item =  '<li>' +
                                '<a href="'+list.Url+'" class="cf external">' +
                        ...
                        $(".Selection").remove();
                        $(".SelectionB").remove();
                        $(".Bottomchose").each(function () {
                            var _type = $(this).attr('data-type');
                            if(_type == '0'){
                                $(this).append('<a href="javascript:void(0)" class="button button-fill pull-right Selection">选择该顾问</a>');
                            }else {
                                $(this).append('<span class="pull-right red SelectionB">已选择该顾问</span>');
                            }
                        })
                        //计算目前所有li的条数
