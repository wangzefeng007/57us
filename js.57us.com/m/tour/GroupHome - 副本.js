...
$(function() {
    ...
    $(document).on('click','.SureBtn',function () {
        ...
        $(".PlayList").empty();
        Load();
        $("#scroller").css('transform','translate(0px, -40px) scale(1) translateZ(0px)');
    })
})

//线路玩法
function PlayFoth(num) {
	myscrollStyle=new iScroll("srcollStyle");
	myscrollCity=new iScroll("srcollCity");
    var tabBox = $(".tabMain .tabBox").eq(num);
    if(tabBox.is('.on')){
        tabBox.removeClass('on');
        $("#StartCity,#EndCity,#WayCity").text('');
        myscrollStyle.refresh();
        myscrollCity.refresh();
    }else {
...
function DataFailure(type) {
    ...
    if(type == 0){
        html = '<div class="payTipBox"  style="margin-top:4.2rem">' +
            ...
            '<a href="#" class="button button-big">重新搜索</a>' +
            '</div>' +
            '</div>';
        $('.PlayList').parent().append(html);
    }else if(type == 1){
        html = '<div class="payTipBox" style="margin-top:4.2rem">' +
            '<p class="tac cf"><i class="icon iconfont sadIcon">&#xe615;</i></p>' +
            '<div class="payContText tac mt10">' +
