...
function StartCity(PlayFoth) {
    $.get('/Templates/M/data/Local/AreaOut.json', function(data) {
        PlayFoth.empty();
        var item;
...
function EndCity(PlayFoth) {
    $.get('/Templates/M/data/Local/AreaEnter.json', function(data) {
        PlayFoth.empty();
        var item;
        $.each(data, function(i, list) {
            item = '<li>' +
...
function WayCity(PlayFoth) {
    ...
    $.get('/Templates/M/data/Local/Citys/'+_thisId+'.json', function(data) {
        if(data != null){
            PlayFoth.empty();
            var item;
            $.each(data, function(i, list) {
...
function Howday(num) {
    ...
    if(tabBox.is('.on')){
        ...
        //行程天数
        $.get('/Templates/M/data/Local/Days.json', function(data) {
            HowdayA.empty();
...
function FeaPlay(num) {
    ...
    if(tabBox.is('.on')){
        ...
        $(".tabMain .tabBox").removeClass('on');
        tabBox.addClass('on');
        $.get('/Templates/M/data/Local/Subject.json', function(data) {
...
function ajaxSubmit(Page) {
    ...
    ajaxData = {
        'Intention': 'GroupLocal',
        'StartCity': $("#ajaxStartCity").text(),	//出发城市
        'EndCity': $("#ajaxEndCity").text(),	//出发城市
...
function DataFailure(type) {
    ...
    if(type == 0){
        html = '<div class="payTipBox" style="margin-top:4.2rem">' +
            ...
            '<a href="/group/local/" class="button button-big external">重新筛选</a>' +
            '</div>' +
            '</div>';
