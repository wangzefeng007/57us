...
function EndCity(PlayFoth) {
    $.get('/Templates/M/data/Feature/AreaEnter.json', function(data) {
        PlayFoth.empty();
        var item;
...
function Howday(num) {
    ...
    if(tabBox.is('.on')){
        ...
        $(".tabMain .tabBox").removeClass('on');
        tabBox.addClass('on');
        var HowdayB = $("#Howday .tabBoxBodyM").eq(0);
...
function FeaPlay(num) {
    ...
    if(tabBox.is('.on')){
        ...
        $(".tabMain .tabBox").removeClass('on');
        tabBox.addClass('on');
        $.get('/Templates/M/data/Feature/Subject.json', function(data) {
                ...
                if(data.ResultCode == '200' || data.ResultCode == '102') {
                    if(data.Data.length) {
                        ...
                        $.each(data.Data, function(i, list) {
                            item = '<li data-id="'+list.TourId+'" class="disable-router">' +
                                '<a href="'+list.TourUrl+'">' +
...
function DataFailure(type) {
    ...
    if(type == 0){
        html = '<div class="payTipBox" style="margin-top:4.2rem">' +
            ...
            '<a href="/play/feature/" class="button button-big external">重新筛选</a>' +
            '</div>' +
            '</div>';
