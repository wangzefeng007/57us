...
function StartCity(PlayFoth) {
    $.get('/Templates/M/data/Daily/AreaOut.json', function(data) {
        PlayFoth.empty();
        var item;
...
function FeaPlay(num) {
    ...
    if(tabBox.is('.on')){
        ...
        $(".tabMain .tabBox").removeClass('on');
        tabBox.addClass('on');
        $.get('/Templates/M/data/Daily/Subject.json', function(data) {
...
function ajaxSubmit(Page) {
    ...
    ajaxData = {
        'Intention': 'PlayDaily',
        'StartCity': $("#ajaxStartCity").text(),	//结束城市
...
function DataFailure(type) {
    ...
    if(type == 0){
        html = '<div class="payTipBox" style="margin-top:4.2rem">' +
            ...
            '<a href="/play/daily/" class="button button-big external">重新筛选</a>' +
            '</div>' +
            '</div>';
