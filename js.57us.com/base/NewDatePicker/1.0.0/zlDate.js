W_creatLink("/base/NewDatePicker/1.0.0/css/datepicker.css");
var obj = { date: new Date(), year: -1, month: -1, priceArr: [],idArr:[] };
...
var pickerEvent = {
    Init: function (elemid) {
        ...
        for (var i = 0; i < tds.length; i++) {
            if (tds[i].getAttribute("date") != null && tds[i].getAttribute("date") != "" && tds[i].getAttribute("price") != "-1" && tds[i].getAttribute("id") != "-2") {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
                    html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" id="' + id + '"><a><span class="date basefix">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
                }
                else{
                    html += '<td  ' + classStyle + ' date="' + obj.year + "-" + obj.month + "-" + (i - c) + '" price="' + price + '" id="' + id + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
                }
                if (index == 6) {
...
var commonUtil = {
    ...
    getId: function (day) {
        ...
        for (var i = 0; i < obj.idArr.length; i++) {
            if (obj.idArr[i].Date == dt) {
                return obj.idArr[i].DayPriceID.split('.')[0];
            }
        }
        return -1;
    },
    chooseClick: function (sender) {
        ...
        if (el != null) {
            ...
            $("#DayPriceID").attr('value',id);
            $("#calendar").removeClass('erro');
