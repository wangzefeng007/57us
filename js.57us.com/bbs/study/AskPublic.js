...
$(function () {
    ...
    $("#TeamSubmit").on('click',function () {
        var ajaxData = {
            'Intention': 'AddQuestion', //方法
            'StandSquare': $('#TeamPositive').val(),//正方内容 AskCategoryID为0时才有
            'StandBack':$("#TeamAnti").val(), //反方内容AskCategoryID为0时才有
            'AskInfo':html_encode($("#TeamText").val()), //提问内容
        }

        if($("#TeamText").val().length < 1){
            layer.msg('问题详情不能为空');
            return
        }else if(ajaxData.StandSquare.length <1){
            layer.msg('正方理由不能为空');
            return
        }else if(ajaxData.StandBack.length <1){
            layer.msg('反方理由不能为空');
            return
        }
    ...
    //我要站队提交
    $("#myTeamSubmit").on('click',function () {
        var ajaxData = {
            'Intention': 'AddAnswer', //方法
            'ID':$("#AskId").val(), //站队ID
            'AnswerInfo':html_encode($("#myTeamText").val()), //支持理由
            'StandInfo':$("#standTeam .support").find('.rb_active').attr('val'), //选择站队 1正方 2反方 没有为0
        }

        if($("#myTeamText").val().length < 1){
            layer.msg('支持理由不能为空');
            return
        }
