...
class StudentManageAjax extends CommonController{
...
//        ini_set('display_errors', '1');
    ...
    public function StudentSureInstruments(){
        ...
        if($Result){
            ...
            $DB->query("ROLLBACK");//判断当执行失败时回滚
            $result_json = array('ResultCode'=>'101','Message'=>'定稿失败,更新调查表状态失败',);
        }
        echo json_encode($result_json);
    }
    ...
    public function StudentApplySchool(){
        ...
        if($Result){
            //更新订单流程表的学校申请状态为3
            $Result1 = $OrderConsultantModule->UpdateInfoByWhere(array('Status'=>3),' OrderID = '.$OrderID .' and Type = 4');
            if($Result1){
    ...
    public function StudentVisa(){
        ...
        $DB->query("BEGIN");//开始事务定义
        //更改订单流程表
        $Result = $OrderConsultantModule->UpdateInfoByWhere(array('Status'=>3),' OrderID = '.$OrderID.' and Type=5');
        if($Result){
            //更新订单表状态为完成
    ...
    public function AmountOperate($ConsultantID,$Amt,$Remarks,$DB){
        ...
        //操作资金
        $BankInfo = MemberService::GetUserBankInfo($ConsultantID);
        //操作后的总金额
        ...
        if($BankIsOk){
            ...
            $BankFlowData = array('FromIP'=>GetIP(),'UserID'=>$ConsultantID,'Amount'=>$Amount,'Amt'=>$Amt,'OperateType'=>4,'Remarks'=>$Remarks,'Type'=>2,'AddTime'=>time(),'PayType'=>0);
