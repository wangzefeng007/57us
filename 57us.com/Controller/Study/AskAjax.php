...
class AskAjax
{
    ...
    public function TopicHot(){
        ...
        if ($_POST){
            $Mysqlwhere  = ' and `Column` = 2 and `Status` = 1 ORDER BY RAND() limit 12';
    ...
    private function AddQuestion(){
        ...
        if($Data['AskCategoryID']==0){
            $Data['IsStand']=1;
            $Data['StandSquare']=$_POST['StandSquare'];
            $Data['StandBack']=$_POST['StandBack'];
            $Data['AskInfo']=$_POST['AskInfo'];
            //过滤敏感字
            if($Data['StandSquare'] === $Data['StandBack']){
                $json_result = array(
                    'ResultCode' => 103,
                    'Message' => '正方反方观点不能一样',
                );
            }
            $Data['StandSquare']  =str_replace($badword,'***', $Data['StandSquare'] );
            $Data['StandBack']  =str_replace($badword,'***', $Data['StandBack'] );
    ...
    private function AddAnswer(){
        ...
        if($_POST['StandInfo']){
            ...
            if($AskID){
                $AskInfoModule=new AskInfoModule();
                $AskInfo=$AskInfoModule->GetInfoByWhere(" and AskID=$AskID and `Status`=1 and IsStand=1");
                if($AskInfo){
                    $AnswerInfo=$AskAnswerInfoModule->GetInfoByWhere(" and AskID=$AskID and UserID=$UserID");
                    if($AnswerInfo){
                        $json_result=array('ResultCode'=>101, 'Message'=>'您已选择过站队,不能重复选择');
                    }else{
                        $Data['AskID']=$AskID;
                        $Data['StandInfo']=$_POST['StandInfo'];
                        $Data['Status']=1;
                        $Data['UserID']=$UserID;
                        $Data['AddTime']=time();
                        //过滤敏感字
                        $Data['AnswerInfo']  =str_replace($badword,'***', $_POST['AnswerInfo'] );
                        $AnswerID=$AskAnswerInfoModule->InsertInfo($Data);
                        if($AnswerID){
                            //更新问题表的AnswerNum、Cent、StandSquareNum或StandBackNum字段
                            $Result = $AskInfoModule->UpdateStandNum($AskID,$Data['StandInfo']);
                            if($Result){
                                $DB->query("COMMIT");//执行事务
                                $json_result = array('ResultCode' => 200, 'Message' => '选择站队成功', 'ID'=>$AnswerID);
                            }
                            else{
                                $DB->query("ROLLBACK");//判断当执行失败时回滚
                                $json_result = array('ResultCode' => 103,'Message' => '选择站队失败,请重新尝试','Error'=>'更新问题表AnswerNum、Cent、StandSquareNum或StandBackNum字段错误');
                            }
                        }else{
                            $DB->query("ROLLBACK");//判断当执行失败时回滚
                            $json_result = array('ResultCode' => 102,'Message' => '选择站队失败,请重新尝试',);
                        }
                    }
                }else{
                    $json_result=array('ResultCode'=>103, 'Message'=>'选择失败,无法选择该站队');
                }
            }else{
                $json_result=array('ResultCode'=>104, 'Message'=>'选择失败,无法选择该站队');
            }
        }else{
            //开启事务
            global $DB;
    ...
    private function GetTag($Info){
        $Tags = array();
        $AskTagModule = new AskTagModule();
        $AskTagList = $AskTagModule->GetInfoByWhere(" and `Column`=2 and `Status`=1 ",true);
