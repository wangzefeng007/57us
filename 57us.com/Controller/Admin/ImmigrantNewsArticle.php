...
class ImmigrantNewsArticle
{
    ...
    public function __construct()
    {
        ...
        include SYSTEM_ROOTPATH . '/Modules/News/Class.TblImmigrationKeywordModule.php';
    ...
    public function Lists()
    {
        ...
        $ListsNum = $TblImmigrationModule->GetListsNum($MysqlWhere);
        ...
        if ($Rscount) {
            ...
            if ($Data['Data']) {
                ...
                foreach ($Data['Data'] as $Key => $Value) {
                    $CategoryInfo = $TblImmigrationCategoryModule->GetInfoByKeyID($Data['Data'][$Key]['CategoryID']);
        ...
        include template("ImmigrantNewsArticleLists");
    }
    ...
    {
        $CategoryList = $this->CategoryLists();
        $ImmigrationID = intval($_GET['ImmigrationID']);
        if ($ImmigrationID) {
            $TblImmigrationModule = new TblImmigrationModule();
            $ImmigrantInfo = $TblImmigrationModule->GetInfoByKeyID($ImmigrationID);
            ...
            foreach ($KeywordInfo as $key=>$value){
                $in=strstr($ImmigrantInfo['Content'],$value['Keyword']);
                if ($in){
                    $Data[$key]['KeyID'] = $value['KeyID'];
                    $Data[$key]['Keyword'] = $value['Keyword'];
                }
            }
            if (strpos($ImmigrantInfo['Keywords'],',')){
                $Keywords =explode(',',$ImmigrantInfo['Keywords']);
            ...
            $ImmigrantInfo['Content'] = StrReplaceImages($ImmigrantInfo['Content']);
            $ImmigrantInfo['Content'] = DoEditorContent($ImmigrantInfo['Content']);
            $ImmigrantInfo['Content']=addslashes($ImmigrantInfo['Content']);
        }
        $TopNavs = "ArticleAdd";
        include template("ImmigrantNewsArticleAdd");
    }
    ...
    {
        $ImmigrationID = intval($_POST['ImmigrationID']);
        ...
        if ($ImmigrationID) {
            if (isset($Data['Image'])) {
                ...
                if ($ImmigrantInfo['Image']) {
                    DelFromImgServ($ImmigrantInfo['Image']);
                }
            }
            $Data['UpdateTime'] = date('Y-m-d H:i:s', $now_time);
            $result = $TblImmigrationModule->UpdateInfoByKeyID($Data, $ImmigrationID);
            if ($result) {
                alertandgotopage('文章保存成功!', "/index.php?Module=ImmigrantNewsArticle&Action=Add&ImmigrationID=$ImmigrationID");
            } else {
            ...
            if ($result) {
                alertandgotopage('文章保存成功!', "/index.php?Module=ImmigrantNewsArticle&Action=Add");
            } else {
                alertandback('文章保存失败,请重新编辑.');
            }
        }
    ...
    {
        $ArticleIDs = $_REQUEST['ImmigrationID'];
        if (!empty($ArticleIDs)) {
            $TblImmigrationModule = new TblImmigrationModule();
            if (is_array($ArticleIDs)) {
                foreach ($ArticleIDs as $ArticleID) {
                    $ImmigrantInfo = $TblImmigrationModule->GetInfoByKeyID($ArticleID);
                    if ($TblImmigrationModule->DeleteByKeyID($ArticleID)) {
                        if ($ImmigrantInfo['Image']) {
                            DelFromImgServ($ImmigrantInfo['Image']);
                        }
                    }
                ...
                $ImmigrantInfo = $TblImmigrationModule->GetInfoByKeyID($ArticleIDs);
                if ($TblImmigrationModule->DeleteByKeyID($ArticleIDs)) {
                    if ($ImmigrantInfo['Image']) {
                        DelFromImgServ($ImmigrantInfo['Image']);
                    }
                    alertandgotopage('已完成删除操作!', $_SERVER['HTTP_REFERER']);
    ...
    public function TagEdit(){
        ...
        if ($_POST){
            ...
            if (empty($ID)){
                ...
                if ($KeyID){
                    alertandgotopage('添加成功!','/index.php?Module=ImmigrantNewsArticle&Action=TagLists');
                }else{
                    alertandback('添加失败!');
                }
            }else{
    ...
    public function DeleteTag(){
        ...
        if ($_GET['ID']) {
            $ID = $_GET['ID'];
            $ImmigrationInfo = $TblImmigrationModule->GetInfoByWhere(' and  MATCH(`Keywords`) AGAINST ('.$ID.' IN BOOLEAN MODE)',true);
            if (!empty($ImmigrationInfo)){
                foreach ($ImmigrationInfo as $key=>$value){
                    ...
                    $UpdateInfo = $TblImmigrationModule->UpdateInfoByKeyID(array('Keywords'=>$In),$value['ImmigrationID']);
                    unset($In);
                }
            }
