    ...
    public function Lists()
    {
        ...
        if ($Rscount['Num']) {
            ...
            foreach ($Data['Data'] as $key => $value) {
                $TourCategory = $TblTourCategoryModule->GetInfoByKeyID($value['CategoryID']);
                $Data['Data'][$key]['CategoryName'] = $TourCategory['CategoryName'];
            }
            $PageMax = $Data['PageCount'];
            MultiPage($Data, 10);
        }
        $TopNavs = 'CaijiTourList';
        include template("CaijiTourList");
    }
    ...
    public function Edit()
    {
        ...
        if ($_POST) {
            ...
            if($submit1){
                ...
                if($result){
                    alertandgotopage('保存成功!',"/index.php?Module=GetTour&Action=Edit&ArticleID=$ArticleID");
                }else{
                ...
                if($Delete){
                    alertandgotopage( '保存并提交成功!', "/index.php?Module=GetTour&Action=Lists");
                }else{
                    alertandback( '保存并提交失败,请重新编辑.' );
                }
            }
        }
        include template("CaijiTourEdit");
    }
