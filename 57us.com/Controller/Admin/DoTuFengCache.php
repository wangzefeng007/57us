...
class DoTuFengCache
{
    ...
    public function UpdatePlayPriceAll()
    {
        global $DB;
        // 当地玩乐价格更新
        $Lists = $DB->select('select TourProductID from tour_product_play_base where Status=0 and SupplierID=' . $this->SupplierID);
        $TourProductIDString = '';
        foreach ($Lists as $Value) {
            $this->UpdatePlayPrice($Value['TourProductID']);
            $TourProductIDString .= $Value['TourProductID'] . ',';
        }
        echo $TourProductIDString;
        exit();
    }
    ...
    public function UpdateSkuInfo($TourProductID = '')
    {
        ...
        if ($PriceText['adult']['price'] == '$0.00' && ! empty($AdultSkuInfo)) {
            $UpdateInfo['Status'] = 0;
        ...
        } elseif ($PriceText['adult']['price'] != '$0.00' && empty($AdultSkuInfo)) {
        ...
        if ($PriceText['kid']['price'] == '$0.00' && ! empty($KidSkuInfo)) {
            $UpdateInfo['Status'] = 0;
        ...
        } elseif ($PriceText['kid']['price'] != '$0.00' && empty($KidSkuInfo)) {
    ...
    private function UpdateErverdayPriceInfo($TourProductID = '')
    {
        ...
        $SkuPriceLists = $DB->select('select * from tour_product_play_sku_price where TourProductID=' . $TourProductID);
