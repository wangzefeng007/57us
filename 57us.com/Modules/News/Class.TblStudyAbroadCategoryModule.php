<?php
	/**
	 * @desc  留学资讯类别表
	 * Class TblStudyAbroadCategoryModule
	 * Author Zf
	 */
class TblStudyAbroadCategoryModule extends  CommonModule {

	public $KeyID = 'CategoryID';
	public $TableName = 'tbl_study_abroad_categories';
	 ...
	 * @return array or false 返回信息
	 */
	public function GetCategoryByID($CategoryID) {
		global $DB;
		$sql = 'select * from `' . $this->TableName . '` where `' . $this->KeyID . '`=' . $CategoryID . '';
		return $DB->getone ( $sql );
	}

	public function GetInfoByParentID($ParentCategoryID) {
		global $DB;
		$sql = 'select CategoryID,CategoryName from  ' . $this->TableName . ' where ParentCategoryID = ' . $ParentCategoryID . '';
